...
public class Mc2 {
	/*******************
	 * 下面4项必须人工填********************* 原则上 service,dao的Spring访问beanName的来源，取表名的除了T开头以外的字符
	...
	public static String modulePack = PropertiesUtil.getKeyValue("mc.package.module");
//			public static String modulePack = StringUtils.isEmpty(PropertiesUtil.getKeyValue("mc.package.module"))
//					? PropertiesUtil.getKeyValue("mc.package.subsystem")
//							: PropertiesUtil.getKeyValue("mc.package.subsystem") + "."
//							+ PropertiesUtil.getKeyValue("mc.package.module");
	...
	public static String[] tmplFileNames = { "J_DB", "J_VO", "J_MAPPER", "J_CUSTOMMAPPER", 
			"mysql".equals(databaseType)?"X_MAPPER_MYSQL":"X_MAPPER_ORACLE",
			"X_CUSTOMMAPPER", "J_EXCHANGER", "J_CONTROL",
			"mysql".equals(databaseType)?"S_SQL_MYSQL":"S_SQL_ORACLE",
			"X_CONFIG", "P_LIST", "P_ADD", "P_VIEW",
			"P_EDIT", "J_SERVICE", "J_SERVICEIMPL", "J_MGRSERVICE", "J_MGRSERVICEIMPL", "J_BIZ", "J_BIZIMPL",
			"J_FACADE", "J_FACADEIMPL", "P_JSON", 
			"J_TEST_BIZ", "J_TEST_CONTROL", "J_TEST_FACADE", "J_TEST_MAPPER", "J_TEST_MGRSERVICE", "J_TEST_SERVICE"};
	...
	public static String[] outFileNames = { "${TABLE_NAME_JAVA}.java", "${TABLE_NAME_JAVA}VO.java",
			...
			"${TABLE_NAME}.sql", "config.txt", "${SUBMODEL_NAME_LOWER}_list.jsp", "${MODULE_NAME_LOWER}_add.jsp",
			"${SUBMODEL_NAME_LOWER}_view.jsp", "${SUBMODEL_NAME_LOWER}_edit.jsp", "${TABLE_NAME_JAVA}Service.java",
			...
			"${TABLE_NAME_JAVA}BizTest.java", "${TABLE_NAME_JAVA}ControllerTest.java", "${TABLE_NAME_JAVA}FacadeTest.java",
			"${TABLE_NAME_JAVA}MapperTest.java", "${TABLE_NAME_JAVA}MgrServiceTest.java", "${TABLE_NAME_JAVA}ServiceTest.java"};
	
	public static String[] packages = { "rpc.api.entity", "rpc.api.dto.vo", "rpc.core.dao", "rpc.core.dao",
			"rpc.sqlmaps", "rpc.sqlmaps", "rpc.core.exchanger", "web.${MODULE_PACKAGE}.controller", "", "",
			"web.${MODULE_NAME_LOWER}", "web.${MODULE_NAME_LOWER}", "zother", "web.${MODULE_NAME_LOWER}",
			"rpc.core.service", "rpc.core.service.impl", "web.${MODULE_PACKAGE}.service",
			"web.${MODULE_PACKAGE}.service.impl", "rpc.core.biz", "rpc.core.biz.impl", "rpc.api.facade",
			"rpc.core.facade.impl", "zother",
			"test.${MODULE_NAME_LOWER}","test.${MODULE_NAME_LOWER}","test.${MODULE_NAME_LOWER}","test.${MODULE_NAME_LOWER}","test.${MODULE_NAME_LOWER}","test.${MODULE_NAME_LOWER}",
	};
					...
					for (int i = 0; i < tmplFileNames.length; i++) {
						...
						String outFile = outFullPath + "/"
								+ outFileNames[i]
										...
										.replace("${SUBMODEL_NAME_JAVA_FU}",
												McUtil.toJavaFirstUpper(table.getSubmodelName()))
